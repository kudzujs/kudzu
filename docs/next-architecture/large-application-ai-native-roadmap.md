...
6. Route/layout capability and CSS chunk closure. **Route/layout CSS closure completed in `0.8.53`; structural capability signatures and emitted chunk reporting completed in `0.8.54`:** each page's reachable TypeScript import/re-export graph supplies exact source stylesheet edges to RouteBuildRecord, configured styles remain global, static siblings exclude unrelated feature CSS, and enhanced navigation loads incoming links before DOM replacement while retaining shared layout link identity and removing outgoing links. `.kudzu/kudzu-artifacts.json` now projects one exact CapabilityIR signature and runtime requirement set per route, follows handler and Worker esbuild metafiles transitively, identifies emitted chunks shared by multiple routes, and supplies the same contract to `afterBuild()`. Emitting signature-keyed runtime families instead of the current site-wide runtime specialization remains.
