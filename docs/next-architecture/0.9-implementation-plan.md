...
| 0.9.0-05 | Active | Execute Session 05B Property Dependency Contract from the pinned WorkLedger failure | None |
...
`0.9.0-01` through `0.9.0-04` and Session 05A are complete. WorkLedger now authorizes object-property component propagation work with one pinned failure spanning an effect dependency and keyed-list consumer. The next session is **0.9.0-05 Session 05B: Property Dependency Contract**.

It:

- identify the parent object `SignalIR`, static `messages` path, specialization owner, effect/list consumers, and selected equality behavior;
- reuse existing Derived property evaluation and `Object.is` effect comparison rather than creating field state;
- retain one logical parent object and reject dynamic paths, mutation, and whole-object dependency ambiguity;
- write the contract before changing `ComponentAnalysis`, ModuleIR, or browser output.
