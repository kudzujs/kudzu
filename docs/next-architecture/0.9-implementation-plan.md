...
**Status:** blocked. Local benchmark source remains ignored by project policy. Native C1 disclosure and the C2/C5 stateful, Worker, navigation, ownership, static-exclusion, resilience, and uniform metric matrices pass. The final 21-profile C2 row filter is faster than Vue, while row restore and search restore are within 3.5% and 1.8% of their nearest comparators. Flattened generated commerce input makes Kudzu 22.8%/19.7% faster than Astro cold/warm at 10,000 products, but Kudzu peak RSS remains release-blocking at 1,870,904 KiB versus 613,916 KiB. Model-driven AI delivery is a 1.0 gate. See [`0.9-final-proof-audit.md`](./0.9-final-proof-audit.md).
...
| 0.9.0-07 | Blocked | Reduce retained 10,000-route plan/artifact memory without dropping evidence | 10,000-route peak RSS is 1,870,904 KiB versus Astro 613,916 KiB |

## Immediate Next Session

`0.9.0-01` through `0.9.0-06` are complete. Native C1, the five-target runtime/capability matrices, C2 browser latency, C5 resource ownership, and 10,000-route commerce build time pass. The next session remains **0.9.0-07**, focused only on the 10,000-route peak-RSS blocker.

It:

- profile retained route-plan, artifact-report, and HTML memory at 10,000 routes;
- reduce peak retention without dropping `.kudzu` evidence or incremental correctness;
- rerun seven rotating 10,000-product samples after the retained optimization;
