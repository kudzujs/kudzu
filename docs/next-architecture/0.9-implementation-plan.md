...
| 0.9.0-05 | Blocked | Find real object-property component propagation failure before Session 05A | No authorizing fixture is pinned |
...
`0.9.0-01` through `0.9.0-04` are complete. Resource work closes with existing `EffectIR`, stronger terminal/WebSocket/package browser evidence, one 12-line lexical-proof deletion, and zero qualifying `ResourceIR` fixtures. Work Item `0.9.0-05` remains blocked until a real application fixture proves object-property component propagation currently requires flattening or artificial primitive state.

It:

- pin the real upstream source, revision, license, and acquisition hash;
- preserve the ordinary component/object-prop boundary in the reduction;
- reproduce the first current compiler failure before semantic work;
- stop rather than invent component semantics if no qualifying fixture exists.
