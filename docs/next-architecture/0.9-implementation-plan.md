...
| 0.9.0-07 | Active | Run the final pinned cross-framework proof from the restored harness | None |

## Immediate Next Session

`0.9.0-01` through `0.9.0-06` are complete. Two independently gated deletions removed 32 core semantic lines, source diagnostics remain stable, and the final complexity review adds no concept, pass, IR kind, normalization entry, or runtime concept. The next session is **0.9.0-07 Final Cross-Framework Proof**.

It:

- verify the restored harness and pinned framework lockfiles before measurement;
- run the complete matched content, stateful, commerce-scale, and resource matrix;
- preserve all Kudzu-only correctness and artifact gates while collecting paired raw samples;
- stop release work if the full benchmark contract or AI migration gate is not reproducible.
