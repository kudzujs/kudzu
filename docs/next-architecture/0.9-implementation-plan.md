...
| 0.9.0-04 | Active | Execute Session 04A lifecycle matrix; default to no ResourceIR | None |
...
`0.9.0-01` through `0.9.0-03` are complete. Context and Zustand action source now converge on package-neutral shared-state/action records and generic handler consumers without changing browser output. The next session is **0.9.0-04 Session 04A: Lifecycle Matrix**.

It:

- inventories acquire, dependencies, owner, invalidation, cleanup, late completion, BFCache, and static exclusion for every existing resource fixture;
- records current executable evidence and gaps before changing semantics;
- defaults to existing `EffectIR` and effect-private ownership;
- adds no `ResourceIR` unless qualifying cross-owner evidence exists.
