...
| 0.9.0-03 | Active | Execute Session 03A equivalence fixture and IR snapshots | A real Context/Zustand duplicate must remain reproducible |
...
`0.9.0-01` and `0.9.0-02` are complete. Medusa proves selected calculation bindings and effects; Mercury proves that the same representation composes with an unrelated financial keyed projection. The next session is **0.9.0-03 Session 03A: Equivalence Fixture And IR Snapshots**.

It:

- identifies the existing real Context and Zustand reductions that exercise equivalent state/action behavior;
- records current ModuleIR and ownership differences before changing generic consumers;
- adds only the smallest failing package-neutral assertion if the duplication remains;
- closes 0.9.0-03 without semantic work if current records are already equivalent enough for generic consumers.
