...
| Stateful | Local five-target matrix passes seven rotating builds/browser profiles, complete frozen C2 acceptance, and uniform artifact/CPU/heap/transfer/build-RSS fields | Ownership-heavy row filter/restore and search restore have material browser latency losses. |
...
The uniform C2 measurements expose material Kudzu losses rather than closing the
release gate: row filter is 34.6 ms versus 10.9-16.7 ms, ownership-heavy row
restore is 208.6 ms versus 50.8-91.9 ms, and search restore is 77.5 ms versus
56.0-67.0 ms. These are accepted behavior samples but release-blocking browser
performance results. C5 repeated Worker processing, reconnect, 30-cycle
...
The runtime and capability raw files now identify code revision `f213e73` and
...
2. Resolve the C2 keyed ownership filter/restore and search-restore browser
   latency losses without weakening release semantics.
3. Run at least seven rotating samples, increasing to twenty-one for overlapping
   timing conclusions, at 100, 1,000, and 10,000 actual emitted routes.
4. Keep model-driven AI migration measurement in the 1.0 work item.
