...
test("explains one emitted route from existing semantic and artifact records", { timeout: 120_000 }, () => {
  const fixture = resolve("test/fixtures/worker-effects")
  const run = route => spawnSync(process.execPath, [cli, "explain", "--route", route, "--json"], { cwd: fixture, encoding: "utf8" })
  const first = run("/dash/dashboard")
  assert.equal(first.status, 0, `${first.stdout}\n${first.stderr}`)
  assert.equal(first.stderr, "")
  const report = JSON.parse(first.stdout)
  assert.equal(report.version, 1)
  assert.equal(report.route, "/dash/dashboard")
  assert.equal(report.entrySource, "src/pages/dashboard.tsx")
  assert.notEqual(report.capability.family, null)
  assert.equal(report.effects.some(effect => effect.owner === "Dashboard" && effect.cleanup && effect.workers.includes("src/telemetry.worker.ts")), true)
  assert.equal(report.artifacts.some(artifact => artifact.reasons.includes("runtime-entry")), true)
  assert.equal(report.artifacts.some(artifact => artifact.reasons.some(reason => reason.startsWith("worker:"))), true)
  assert.equal(report.artifacts.every(artifact => artifact.external || artifact.rawBytes > 0 && artifact.gzipBytes > 0 && /^[a-f0-9]{64}$/.test(artifact.sha256)), true)
  assert.equal(report.zeroJavaScript.value, false)
  assert.doesNotMatch(first.stdout, /moduleIR|\"plan\"|\"html\"|\"code\"|captures/)
  assert.doesNotMatch(first.stdout, new RegExp(fixture.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")))
  assert.equal(run("/dash/dashboard").stdout, first.stdout)

  const staticResult = run("/dash/static")
  assert.equal(staticResult.status, 0, `${staticResult.stdout}\n${staticResult.stderr}`)
  const staticReport = JSON.parse(staticResult.stdout)
  assert.equal(staticReport.entrySource, "src/pages/static.tsx")
  assert.equal(staticReport.capability.family, null)
  assert.equal(staticReport.bytes.javascriptRawBytes, 0)
  assert.equal(staticReport.zeroJavaScript.value, true)
  assert.deepEqual(staticReport.artifacts, [])

  const missing = run("/dashboard")
  assert.equal(missing.status, 1)
  assert.equal(missing.stdout, "")
  const diagnostic = JSON.parse(missing.stderr)
  assert.equal(diagnostic.diagnostics[0].code, "explain.route.not-found")
  assert.equal(diagnostic.diagnostics[0].stage, "explain")
})
