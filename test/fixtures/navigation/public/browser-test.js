...
if (sessionStorage.kNavigationFallback) {
  ...
  try {
    ...
    document.querySelector("[data-row-late]").click()
    await waitFor(() => document.body.nativeRowResolvers?.length === 1)
    document.querySelector("[data-remove-native-row]").click()
    await waitFor(() => !document.querySelector('[data-item="vine"]'))
    document.querySelector("[data-restore-native-row]").click()
    await waitFor(() => document.querySelector('[data-item="vine"] [data-row-count]')?.textContent === "0")
    document.body.nativeRowResolvers.shift()()
    await new Promise(resolve => setTimeout(resolve, 0))
    if (document.querySelector('[data-item="vine"] [data-row-count]').textContent !== "0" || document.querySelector('[data-item="vine"] [data-row-count]').hasAttribute("data-resolved")) throw new Error("stale-native-row")
    document.querySelector('[data-item="vine"] [data-row-late]').click()
    await waitFor(() => document.body.nativeRowResolvers?.length === 1)
    document.body.nativeRowResolvers.shift()()
    await waitFor(() => document.querySelector('[data-item="vine"] [data-row-count]').textContent === "1" && document.querySelector('[data-item="vine"] [data-row-count]').hasAttribute("data-resolved"), "fresh-native-row")
    document.querySelector("[data-route-late]").click()
    await waitFor(() => document.body.nativeRouteResolvers?.length === 1)
    ...
    document.body.nativeRouteResolvers.shift()()
    await new Promise(resolve => setTimeout(resolve, 0))
    if (document.querySelector("[data-route-count]").textContent !== "Route 0" || document.querySelector("[data-route-ref]").hasAttribute("data-resolved")) throw new Error("stale-native-route")
    history.forward()
    await waitFor(() => document.querySelector('[data-route="cart"]'))
    ...
    document.querySelector("[data-layout-late]").click()
    await waitFor(() => document.body.nativeDisposalResolver, "native-disposal-start")
    pagehide(false)
    document.body.nativeDisposalResolver()
    await new Promise(resolve => setTimeout(resolve, 0))
    if (document.querySelector("[data-layout-count]").textContent !== "Layout 2" || document.querySelector("[data-layout-ref]").hasAttribute("data-resolved")) throw new Error("native-disposal-late-write")
