...
try {
  if (location.pathname === "/app/alpha") {
    ...
    if (document.querySelector('[data-shell="a"]') !== shell || document.querySelector('[data-route="item"]') !== item || location.search !== "?view=compact") throw new Error("query-write")
    ...
    await waitFor(() => document.querySelector('[data-child="a1"]')?.textContent === "AshUnavailable")
    if (document.querySelector('[data-child="a1"]')?.title !== "Ash") throw new Error("nested-navigation-update")
