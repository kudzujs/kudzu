...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.4/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.4"/)
  assert.match(release, /Kudzu 0\.8\.4.*Keep globals.*In the browser/s)
  assert.match(release, /BROWSER-NATIVE HANDLERS.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.8\.4/s)
