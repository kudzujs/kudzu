...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.13.2/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.13\.2 typescript/)
  assert.match(docs, /Current 0\.13\.2 application proof/)
  assert.match(examples, /Release 0\.13\.2.*v0\.13\.2.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.13\.2.*v0\.13\.2.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.13\.2".*Multistep draft and autosave/s)
  assert.match(release, /Kudzu 0\.13\.2.*Keep the draft.*Reject the stale save/s)
  assert.match(release, /STEPS · VERSIONS · OWNED EFFECTS.*DRAFT OWNERSHIP.*npm install @kudzujs\/core@\^0\.13\.2/s)
  assert.match(release, /<title>Kudzu 0\.13\.2 - Multistep draft and autosave<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.13\.2"/)
  assert.match(release, /Use native constraints.*Clear every owner/s)
