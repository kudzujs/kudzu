...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.53/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.53"/)
  assert.match(release, /Kudzu 0\.8\.53.*Style the route.*Not the whole site/s)
  assert.match(release, /REACH · LOAD · RETAIN.*ROUTE\/LAYOUT CSS CLOSURE.*npm install @kudzujs\/core@\^0\.8\.53/s)
  assert.match(release, /<title>Kudzu 0\.8\.53 - Route-aware CSS closure<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.53"/)
  assert.match(release, /Follow imports.*Rollback safely/s)
