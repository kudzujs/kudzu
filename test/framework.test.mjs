...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.13.3/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.13\.3 typescript/)
  assert.match(docs, /Current 0\.13\.3 application proof/)
  assert.match(examples, /Release 0\.13\.3.*v0\.13\.3.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.13\.3.*v0\.13\.3.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.13\.3".*File upload boundary/s)
  assert.match(release, /Kudzu 0\.13\.3.*Upload the file.*Abort with the owner/s)
  assert.match(release, /FILES · CANCELLATION · OWNED EFFECTS.*UPLOAD OWNERSHIP.*npm install @kudzujs\/core@\^0\.13\.3/s)
  assert.match(release, /<title>Kudzu 0\.13\.3 - File upload boundary<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.13\.3"/)
  assert.match(release, /Reject before request.*Append the attachment/s)
