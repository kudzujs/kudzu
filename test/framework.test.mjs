...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.13.1/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.13\.1 typescript/)
  assert.match(docs, /Current 0\.13\.1 application proof/)
  assert.match(examples, /Release 0\.13\.1.*v0\.13\.1.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.13\.1.*v0\.13\.1.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.13\.1".*Nested form metadata/s)
  assert.match(release, /Kudzu 0\.13\.1.*Own the metadata.*Keep the field identity/s)
  assert.match(release, /STATE · KEYS · NATIVE RESET.*FORM METADATA.*npm install @kudzujs\/core@\^0\.13\.1/s)
  assert.match(release, /<title>Kudzu 0\.13\.1 - Nested form metadata<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.13\.1"/)
  assert.match(release, /Own one object.*Ship no proxy graph/s)
