...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.23/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.23 typescript/)
  assert.match(docs, /Current 0\.16\.23 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*11\/25 Kudzu successes.*24\/25 for React \+ Vite.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.23.*v0\.16\.23.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.23.*v0\.16\.23.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.23".*Reactive conditional branch text/s)
  assert.match(release, /Kudzu 0\.16\.23.*Keep the branch.*Update the words/s)
  assert.match(release, /ORDINARY TSX · REACTIVE TEXT · EXISTING OWNERSHIP.*REACTIVE CONDITIONAL BRANCH TEXT.*npm install @kudzujs\/core@\^0\.16\.23/s)
  assert.match(release, /<title>Kudzu 0\.16\.23 - Reactive conditional branch text<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.23"/)
  assert.match(release, /Scalar branches.*Stable branch.*Commerce 5\/5/s)
