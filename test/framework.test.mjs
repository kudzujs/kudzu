...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.30 typescript/)
  assert.match(docs, /Current 0\.16\.30 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*23\/25 Kudzu successes.*24\/25 for React \+ Vite.*50 attributable attempts.*Installed source and public web access.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.30.*v0\.16\.30.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.30.*v0\.16\.30.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.30".*Binding target specialization/s)
