...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.11.1/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.11\.1".*List\/detail data consistency/s)
  assert.match(release, /Kudzu 0\.11\.1.*Share the record.*Count every request/s)
  assert.match(release, /READ · MUTATE · RELOAD.*SHARED SERVER DATA.*npm install @kudzujs\/core@\^0\.11\.1/s)
  assert.match(release, /<title>Kudzu 0\.11\.1 - List\/detail data consistency<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.11\.1"/)
  assert.match(release, /Retain one owner.*Skip query runtimes/s)
