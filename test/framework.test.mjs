...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.41/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.41"/)
  assert.match(release, /Kudzu 0\.8\.41.*Cross one more boundary.*Keep ownership direct/s)
  assert.match(release, /FORWARD · SPECIALIZE · RELEASE.*CALLBACK OWNERSHIP.*npm install @kudzujs\/core@\^0\.8\.41/s)
  assert.match(release, /<title>Kudzu 0\.8\.41 - Two-boundary callback ownership<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.41"/)
  assert.match(release, /Preserve parent signals.*Reject a third boundary/s)
...
test("owns setter callbacks and object refs across two component boundaries", async t => {
  ...
  assert.doesNotMatch(emitted, /function (?:ImportedControls|ImportedButton|ImportedSearchField|ImportedSearch)\b|["']react["']/)
...
test("rejects setter callbacks across a third component boundary", () => {
  const fixture = new URL("./fixtures/setter-third-boundary-invalid", import.meta.url)
  const result = spawnSync(process.execPath, [new URL("../bin/kudzu.mjs", import.meta.url).pathname, "build"], { cwd: fixture, encoding: "utf8" })
  assert.notEqual(result.status, 0)
  assert.match(`${result.stdout}\n${result.stderr}`, /src\/pages\/index\.tsx:\d+:\d+ Setter callbacks cannot cross more than two component boundaries/)
})
