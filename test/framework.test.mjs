...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.49/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.49"/)
  assert.match(release, /Kudzu 0\.8\.49.*Forward one callback.*Connect every control/s)
  assert.match(release, /BRANCH · SPECIALIZE · OWN.*CALLBACK FAN-OUT.*npm install @kudzujs\/core@\^0\.8\.49/s)
  assert.match(release, /<title>Kudzu 0\.8\.49 - Setter callback fan-out<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.49"/)
  assert.match(release, /Forward directly.*Ship no callbacks/s)
