...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.12.0/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.12\.0 typescript/)
  assert.match(docs, /Current 0\.12\.0 application proof/)
  assert.match(examples, /Release 0\.12\.0.*v0\.12\.0.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.12\.0.*v0\.12\.0.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.12\.0".*Project route shell and runtime parameters/s)
  assert.match(release, /Kudzu 0\.12\.0.*Address every route.*Keep navigation native/s)
  assert.match(release, /PATH · DOCUMENT · RELEASE.*DIRECT ROUTES.*npm install @kudzujs\/core@\^0\.12\.0/s)
  assert.match(release, /<title>Kudzu 0\.12\.0 - Project route shell and runtime parameters<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.12\.0"/)
  assert.match(release, /Order rewrites safely.*Skip the router runtime/s)
