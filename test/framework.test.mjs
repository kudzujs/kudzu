...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.12.2/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.12\.2 typescript/)
  assert.match(docs, /Current 0\.12\.2 application proof/)
  assert.match(examples, /Release 0\.12\.2.*v0\.12\.2.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.12\.2.*v0\.12\.2.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.12\.2".*Authentication and permission boundary/s)
  assert.match(release, /Kudzu 0\.12\.2.*Show the role.*Enforce the boundary/s)
  assert.match(release, /SESSION · PERMISSION · RELEASE.*AUTH OWNERSHIP.*npm install @kudzujs\/core@\^0\.12\.2/s)
  assert.match(release, /<title>Kudzu 0\.12\.2 - Authentication and permission boundary<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.12\.2"/)
  assert.match(release, /Use native login.*Authorize every action/s)
