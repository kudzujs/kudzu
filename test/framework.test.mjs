...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.27/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.27 typescript/)
  assert.match(docs, /Current 0\.16\.27 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*23\/25 Kudzu successes.*24\/25 for React \+ Vite.*50 attributable attempts.*Installed source and public web access.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.27.*v0\.16\.27.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.27.*v0\.16\.27.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.27".*Application verification guidance/s)
  assert.match(release, /Kudzu 0\.16\.27.*Build the output.*Verify the application/s)
  assert.match(release, /APPLICATION VERIFICATION.*npm install @kudzujs\/core@\^0\.16\.27/s)
  assert.match(release, /<title>Kudzu 0\.16\.27 - Application verification guidance<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.27"/)
  assert.match(release, /Visible DOM, not raw strings.*Optional reports rebuild.*A bounded success hint.*Historical evidence, not savings/s)
