...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.13.0/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.13\.0 typescript/)
  assert.match(docs, /Current 0\.13\.0 application proof/)
  assert.match(examples, /Release 0\.13\.0.*v0\.13\.0.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.13\.0.*v0\.13\.0.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.13\.0".*Production form and server validation/s)
  assert.match(release, /Kudzu 0\.13\.0.*Keep the form native.*Own only what the app needs/s)
  assert.match(release, /NATIVE · ACCESSIBLE · RETRYABLE.*FORM OWNERSHIP.*npm install @kudzujs\/core@\^0\.13\.0/s)
  assert.match(release, /<title>Kudzu 0\.13\.0 - Production form and server validation<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.13\.0"/)
  assert.match(release, /Let the browser block.*Ship no registry/s)
