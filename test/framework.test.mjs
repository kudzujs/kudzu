...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.35/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.35"/)
  assert.match(release, /Kudzu 0\.8\.35.*Names explain.*Identity decides/s)
  assert.match(release, /SYMBOLS · SITES · RESOLVE.*SEMANTIC IDENTITY.*npm install @kudzujs\/core@\^0\.8\.35/s)
  assert.match(release, /<title>Kudzu 0\.8\.35 - Stable module identity<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.35"/)
  assert.match(release, /Declarations keep one identity.*Semantic state operations/s)
