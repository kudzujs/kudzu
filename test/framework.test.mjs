...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.12/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.12"/)
  assert.match(release, /Kudzu 0\.8\.12.*Keep icons familiar.*Ship only the SVG/s)
  assert.match(release, /STATIC ICON MIGRATION.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.8\.12/s)
