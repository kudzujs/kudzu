...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.12/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.12 typescript/)
  assert.match(docs, /Current 0\.16\.12 inspection evidence.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.12.*v0\.16\.12.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.12.*v0\.16\.12.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.12".*Bounded application inspection/s)
  assert.match(release, /Kudzu 0\.16\.12.*Inspect the application.*Skip the raw compiler/s)
  assert.match(release, /REACHABLE FACTS · FIXED LIMITS · FIRST BLOCKER RETAINED.*APPLICATION INSPECTION.*npm install @kudzujs\/core@\^0\.16\.12/s)
  assert.match(release, /<title>Kudzu 0\.16\.12 - Bounded application inspection<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.12"/)
  assert.match(release, /Report only used source.*Retain the first stop.*Sort before truncation/s)
