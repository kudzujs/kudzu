...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.7.20/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.7\.20"/)
  assert.match(release, /Kudzu 0\.7\.20.*Compute the children.*Keep the keys/s)
  assert.match(release, /COMPUTED CHILD COLLECTIONS.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.7\.20/s)
