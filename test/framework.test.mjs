...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.14/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.14 typescript/)
  assert.match(docs, /Current 0\.16\.14 delivery evidence.*benchmark:ai-delivery.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.14.*v0\.16\.14.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.14.*v0\.16\.14.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.14".*Fair AI delivery evidence/s)
  assert.match(release, /Kudzu 0\.16\.14.*Keep every attempt.*Measure before claiming/s)
  assert.match(release, /EQUAL CONDITIONS · RAW FAILURES · ATTRIBUTED COST.*AI DELIVERY PROTOCOL.*npm install @kudzujs\/core@\^0\.16\.14/s)
  assert.match(release, /<title>Kudzu 0\.16\.14 - Fair AI delivery evidence<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.14"/)
  assert.match(release, /Share one protocol.*Keep failed work.*Claim only protocol proof/s)
