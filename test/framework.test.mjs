...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.11.2/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.11\.2".*Request coordination evidence/s)
  assert.match(release, /Kudzu 0\.11\.2.*Review the pressure.*Keep one owner/s)
  assert.match(release, /OWN · INVALIDATE · RELEASE.*COORDINATION DECISION.*npm install @kudzujs\/core@\^0\.11\.2/s)
  assert.match(release, /<title>Kudzu 0\.11\.2 - Request coordination evidence<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.11\.2"/)
  assert.match(release, /Reuse owned effects.*Skip request registries/s)
