...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.11.3/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.11\.3 typescript/)
  assert.match(docs, /Current 0\.11\.3 application proof/)
  assert.match(examples, /Release 0\.11\.3.*v0\.11\.3.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.11\.3.*v0\.11\.3.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.11\.3".*Optimistic mutation and rollback/s)
  assert.match(release, /Kudzu 0\.11\.3.*Write first.*Rollback exactly/s)
  assert.match(release, /WRITE · REJECT · RETRY.*OWNED MUTATION.*npm install @kudzujs\/core@\^0\.11\.3/s)
  assert.match(release, /<title>Kudzu 0\.11\.3 - Optimistic mutation and rollback<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.11\.3"/)
  assert.match(release, /Disable duplicates.*Skip transactions/s)
