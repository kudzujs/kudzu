...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.15/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.15"/)
  assert.match(release, /Kudzu 0\.8\.15.*React-shaped input.*Explicit compiler stages/s)
  assert.match(release, /COMPILER ARCHITECTURE.*WHAT CHANGED.*npm install @kudzujs\/core@\^0\.8\.15/s)
