...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.5/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.5"/)
  assert.match(release, /Kudzu 0\.8\.5.*Replace the timer.*Keep the owner/s)
  assert.match(release, /OWNED TIMER ACTIONS.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.8\.5/s)
