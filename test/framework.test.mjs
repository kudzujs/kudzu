...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.7.4/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.7\.4"/)
  assert.match(release, /Kudzu 0\.7\.4.*Keep the pipeline.*Preserve the rows/s)
  assert.match(release, /MEMOIZED COLLECTION PIPELINES.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.7\.4/s)
...
test("migrates aliased and member React hooks from a Vite-shaped app", async t => {
  ...
  assert.doesNotMatch(component, /\b(?:useCallback|useMemo|useMenuState|runEffect|preserve|derive)\b|React\.useState/)
  assert.match(component, /useState\(false, "menuOpen"\)/)
