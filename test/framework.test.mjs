...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.18/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.18 typescript/)
  assert.match(docs, /Current 0\.16\.18 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*unattributed AI attempts.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.18.*v0\.16\.18.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.18.*v0\.16\.18.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.18".*AI attribution integrity/s)
  assert.match(release, /Kudzu 0\.16\.18.*Count the evidence.*Never the assumption/s)
  assert.match(release, /RAW TRACE REQUIRED · ZERO FALSE SUCCESSES · ZERO RUNTIME DELTA.*AI ATTRIBUTION INTEGRITY.*npm install @kudzujs\/core@\^0\.16\.18/s)
  assert.match(release, /<title>Kudzu 0\.16\.18 - AI attribution integrity<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.18"/)
  assert.match(release, /No synthetic success.*Keep failures visible.*Withhold unsupported claims/s)
