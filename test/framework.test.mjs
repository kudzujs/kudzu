...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.10.2/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.10\.2".*Shared-layout navigation/s)
  assert.match(release, /Kudzu 0\.10\.2.*Keep the workspace.*Release the route/s)
  assert.match(release, /LAYOUT · ROUTE · RELEASE.*SHARED LAYOUT.*npm install @kudzujs\/core@\^0\.10\.2/s)
  assert.match(release, /<title>Kudzu 0\.10\.2 - Shared-layout navigation<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.10\.2"/)
  assert.match(release, /Keep workspace state.*Skip the SPA router/s)
