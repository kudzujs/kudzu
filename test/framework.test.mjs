...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.10.1/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.10\.1".*Application state scale/s)
  assert.match(release, /Kudzu 0\.10\.1.*Grow the state graph.*Keep the boundary exact/s)
  assert.match(release, /STATE · OWNERSHIP · SCALE.*APPLICATION STATE.*npm install @kudzujs\/core@\^0\.10\.1/s)
  assert.match(release, /<title>Kudzu 0\.10\.1 - Application state scale<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.10\.1"/)
  assert.match(release, /Use ordinary records.*Stop before a runtime/s)
