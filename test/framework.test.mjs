...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.34/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.34"/)
  assert.match(release, /Kudzu 0\.8\.34.*Parse once.*Transform in isolation/s)
  assert.match(release, /PARSE · SUMMARIZE · CLONE.*MODULE OWNERSHIP.*npm install @kudzujs\/core@\^0\.8\.34/s)
  assert.match(release, /<title>Kudzu 0\.8\.34 - Session-local module cache<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.34"/)
  assert.match(release, /Unique modules define the work.*Stable semantic identity/s)
