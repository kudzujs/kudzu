...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.11/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.11"/)
  assert.match(release, /Kudzu 0\.8\.11.*Move data by timing.*Keep only the latest/s)
  assert.match(release, /STALE-SAFE DATA MIGRATION.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.8\.11/s)
