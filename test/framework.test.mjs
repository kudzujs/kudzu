...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.14.2/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.14\.2 typescript/)
  assert.match(docs, /Current 0\.14\.2 application proof/)
  assert.match(examples, /Release 0\.14\.2.*v0\.14\.2.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.14\.2.*v0\.14\.2.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.14\.2".*Infinite loading composition/s)
  assert.match(release, /Kudzu 0\.14\.2.*Reach the sentinel.*Load only the next page/s)
  assert.match(release, /SENTINEL · OWNED FETCH · BOUNDED RESULTS.*INCREMENTAL OWNERSHIP.*npm install @kudzujs\/core@\^0\.14\.2/s)
  assert.match(release, /<title>Kudzu 0\.14\.2 - Infinite loading composition<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.14\.2"/)
  assert.match(release, /Use the platform.*Stop at six/s)
