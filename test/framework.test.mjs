...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.3/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.3"/)
  assert.match(release, /Kudzu 0\.8\.3.*Use the platform.*Keep ownership explicit/s)
  assert.match(release, /NATIVE INTERACTION COMPOSITION.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.8\.3/s)
