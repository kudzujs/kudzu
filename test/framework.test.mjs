...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.7.28/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.7\.28"/)
  assert.match(release, /Kudzu 0\.7\.28.*Nest the component.*Erase the boundary/s)
  assert.match(release, /NESTED COMPONENT SPECIALIZATION.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.7\.28/s)
