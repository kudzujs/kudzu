...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.7.3/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.7\.3"/)
  assert.match(release, /Kudzu 0\.7\.3.*Keep memo syntax.*Ship reactive HTML/s)
  assert.match(release, /REACT MEMO NORMALIZATION.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.7\.3/s)
...
test("migrates aliased and member React hooks from a Vite-shaped app", async t => {
  ...
  assert.doesNotMatch(component, /useCallback|useMemo|\bmemo\b|useMenuState|runEffect|preserve|derive|React\.useState/)
  assert.match(component, /useState\(false, "menuOpen"\)/)
  assert.match(component, /useState\(0, "count"\)/)
  assert.match(component, /const doubled = count \* 2/)
  assert.doesNotMatch(staticHtml, /<script/)
