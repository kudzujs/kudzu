...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.40/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.40"/)
  assert.match(release, /Kudzu 0\.8\.40.*Subscribe to the object.*React to the property/s)
  assert.match(release, /SELECT · COMPARE · REACT.*PROPERTY DEPENDENCIES.*npm install @kudzujs\/core@\^0\.8\.40/s)
  assert.match(release, /<title>Kudzu 0\.8\.40 - Property-level effect dependencies<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.40"/)
  assert.match(release, /Keep ordinary object state.*Cross more component boundaries/s)
