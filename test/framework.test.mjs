...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.50/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.50"/)
  assert.match(release, /Kudzu 0\.8\.50.*Contain package syntax.*Share one state model/s)
  assert.match(release, /ADAPT · VALIDATE · LOWER.*SHARED STATE IR.*npm install @kudzujs\/core@\^0\.8\.50/s)
  assert.match(release, /<title>Kudzu 0\.8\.50 - Package-neutral shared state IR<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.50"/)
  assert.match(release, /Contain Zustand.*Publish no store API/s)
