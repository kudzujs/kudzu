...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.33/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.33"/)
  assert.match(release, /Kudzu 0\.8\.33.*One process.*Independent projects/s)
  assert.match(release, /ROOT · GRAPH · OUTPUT.*PROJECT OWNERSHIP.*npm install @kudzujs\/core@\^0\.8\.33/s)
  assert.match(release, /<title>Kudzu 0\.8\.33 - Project-scoped compilation<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.33"/)
  assert.match(release, /Same names stay separate.*Parse once per session/s)
