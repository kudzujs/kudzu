...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.15/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.15 typescript/)
  assert.match(docs, /Current 0\.16\.15 production evidence.*benchmark:source-scale.*benchmark:ai-delivery.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.15.*v0\.16\.15.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.15.*v0\.16\.15.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.15".*Production evidence gates/s)
  assert.match(release, /Kudzu 0\.16\.15.*Measure the real path.*Reject incomplete proof/s)
  assert.match(release, /COMPILER SCALE · BROWSER PARITY · COMPLETE ATTRIBUTION.*PRODUCTION EVIDENCE.*npm install @kudzujs\/core@\^0\.16\.15/s)
  assert.match(release, /<title>Kudzu 0\.16\.15 - Production evidence gates<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.15"/)
  assert.match(release, /Time compiler boundaries.*Compare clean output.*Name the remaining scale/s)
