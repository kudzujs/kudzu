...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.11.4/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.11\.4 typescript/)
  assert.match(docs, /Current 0\.11\.4 application proof/)
  assert.match(examples, /Release 0\.11\.4.*v0\.11\.4.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.11\.4.*v0\.11\.4.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.11\.4".*Pagination, refresh, and polling policy/s)
  assert.match(release, /Kudzu 0\.11\.4.*Bound the data.*Own the policy/s)
  assert.match(release, /PAGE · REFRESH · RELEASE.*OWNED SERVER DATA.*npm install @kudzujs\/core@\^0\.11\.4/s)
  assert.match(release, /<title>Kudzu 0\.11\.4 - Pagination, refresh, and polling policy<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.11\.4"/)
  assert.match(release, /Suppress hidden work.*Skip schedulers/s)
