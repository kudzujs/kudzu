...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.45/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.45"/)
  assert.match(release, /Kudzu 0\.8\.45.*Skip work that plain TypeScript.*does not need/s)
  assert.match(release, /CLASSIFY · TRANSPILE · SHIP.*SOURCE SCALE.*npm install @kudzujs\/core@\^0\.8\.45/s)
  assert.match(release, /<title>Kudzu 0\.8\.45 - Plain TypeScript fast path<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.45"/)
  assert.match(release, /Use the short path.*Ship nothing new/s)
