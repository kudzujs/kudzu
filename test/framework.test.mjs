...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.10.0/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.10\.0"/)
  assert.match(release, /Kudzu 0\.10\.0.*Prove the application.*Then extend the compiler/s)
  assert.match(release, /MODEL · BUILD · VERIFY.*APPLICATION BASELINE.*npm install @kudzujs\/core@\^0\.10\.0/s)
  assert.match(release, /<title>Kudzu 0\.10\.0 - Application capability baseline<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.10\.0"/)
  assert.match(release, /Track one greenfield surface.*Release by capability/s)
