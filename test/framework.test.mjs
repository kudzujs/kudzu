...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.19/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.19 typescript/)
  assert.match(docs, /Current 0\.16\.19 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*11\/25 Kudzu successes.*24\/25 for React \+ Vite.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.19.*v0\.16\.19.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.19.*v0\.16\.19.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.19".*Corrected AI delivery evidence/s)
  assert.match(release, /Kudzu 0\.16\.19.*Publish the result.*Especially when it loses/s)
  assert.match(release, /50 PREDECLARED ATTEMPTS · FAILED 1\.0 GATE · ZERO RUNTIME DELTA.*CORRECTED AI DELIVERY PROOF.*npm install @kudzujs\/core@\^0\.16\.19/s)
  assert.match(release, /<title>Kudzu 0\.16\.19 - Corrected AI delivery evidence<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.19"/)
  assert.match(release, /Gate not met.*No aggregate claim.*Accepted, over budget/s)
