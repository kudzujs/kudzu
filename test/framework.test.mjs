...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.12.3/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.12\.3 typescript/)
  assert.match(docs, /Current 0\.12\.3 application proof/)
  assert.match(examples, /Release 0\.12\.3.*v0\.12\.3.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.12\.3.*v0\.12\.3.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.12\.3".*Route failure and restoration policy/s)
  assert.match(release, /Kudzu 0\.12\.3.*Keep the document.*Restore the path/s)
  assert.match(release, /RETAIN · RESTORE · RETRY.*FAILURE OWNERSHIP.*npm install @kudzujs\/core@\^0\.12\.3/s)
  assert.match(release, /<title>Kudzu 0\.12\.3 - Route failure and restoration policy<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.12\.3"/)
  assert.match(release, /Keep the document.*Reject invalid assets/s)
