...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.25/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.25 typescript/)
  assert.match(docs, /Current 0\.16\.25 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*11\/25 Kudzu successes.*24\/25 for React \+ Vite.*18\/25 versus 23\/25.*two partial traces.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.25.*v0\.16\.25.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.25.*v0\.16\.25.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.25".*Static content and acceptance alignment/s)
  assert.match(release, /Kudzu 0\.16\.25.*Keep static content.*Drop its JavaScript/s)
  assert.match(release, /STATIC CONTENT.*npm install @kudzujs\/core@\^0\.16\.25/s)
  assert.match(release, /<title>Kudzu 0\.16\.25 - Static content and acceptance alignment<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.25"/)
  assert.match(release, /Static selectors.*Smaller output.*Semantic requirements.*No replacement score/s)
