...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.17/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.17 typescript/)
  assert.match(docs, /Current 0\.16\.17 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.17.*v0\.16\.17.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.17.*v0\.16\.17.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.17".*Browser performance gates/s)
  assert.match(release, /Kudzu 0\.16\.17.*Measure the session.*Fail when it drifts/s)
  assert.match(release, /100 MS ACTIONS · 101 NAVIGATIONS · ZERO RUNTIME DELTA.*BROWSER PERFORMANCE GATES.*npm install @kudzujs\/core@\^0\.16\.17/s)
  assert.match(release, /<title>Kudzu 0\.16\.17 - Browser performance gates<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.17"/)
  assert.match(release, /Restore under budget.*Keep the DOM bounded.*Preserve the layout/s)
