...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.11.0/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.11\.0".*Owned fetch lifecycle/s)
  assert.match(release, /Kudzu 0\.11\.0.*Fetch the data.*Own the lifecycle/s)
  assert.match(release, /LOAD · REPLACE · RELEASE.*OWNED SERVER DATA.*npm install @kudzujs\/core@\^0\.11\.0/s)
  assert.match(release, /<title>Kudzu 0\.11\.0 - Owned fetch lifecycle<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.11\.0"/)
  assert.match(release, /Show loading accessibly.*Skip query runtimes/s)
