...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.38/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.38"/)
  assert.match(release, /Kudzu 0\.8\.38.*Render the route.*Follow its edges/s)
  assert.match(release, /ROUTE · EDGE · EMIT.*ARTIFACT GRAPH.*npm install @kudzujs\/core@\^0\.8\.38/s)
  assert.match(release, /<title>Kudzu 0\.8\.38 - Structural route artifacts<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.38"/)
  assert.match(release, /Only rendered descriptors retain code.*Validate RouteIR deeply/s)
