...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.7.27/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.7\.27"/)
  assert.match(release, /Kudzu 0\.7\.27.*Keep the hooks.*Lose the runtime/s)
  assert.match(release, /STATEFUL COMPONENT MIGRATION.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.7\.27/s)
