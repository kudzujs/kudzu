...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.26/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.26 typescript/)
  assert.match(docs, /Current 0\.16\.26 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*20\/25 Kudzu successes.*25\/25 for React \+ Vite.*50 attributable attempts.*Installed evidence\/compiler reads.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.26.*v0\.16\.26.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.26.*v0\.16\.26.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.26".*Product documentation and package hygiene/s)
  assert.match(release, /Kudzu 0\.16\.26.*Keep the product.*Leave internal docs out/s)
  assert.match(release, /PACKAGE HYGIENE.*npm install @kudzujs\/core@\^0\.16\.26/s)
  assert.match(release, /<title>Kudzu 0\.16\.26 - Product documentation and package hygiene<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.26"/)
  assert.match(release, /Supported patterns.*Actual tarball checks.*Historical package comparison.*Evidence stays honest/s)
