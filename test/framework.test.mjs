...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.1/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.1"/)
  assert.match(release, /Kudzu 0\.8\.1.*Keep reset ownership.*Inside the hook/s)
  assert.match(release, /CUSTOM-HOOK RESET ACTIONS.*WHAT LANDED.*npm install @kudzujs\/core@\^0\.8\.1/s)
