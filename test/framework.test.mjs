...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.12.4/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.12\.4 typescript/)
  assert.match(docs, /Current 0\.12\.4 application proof/)
  assert.match(examples, /Release 0\.12\.4.*v0\.12\.4.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.12\.4.*v0\.12\.4.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.12\.4".*Nested layout evidence decision/s)
  assert.match(release, /Kudzu 0\.12\.4.*Keep one owner.*Until evidence says otherwise/s)
  assert.match(release, /EVIDENCE · OWNERSHIP · RESTRAINT.*LAYOUT OWNERSHIP.*npm install @kudzujs\/core@\^0\.12\.4/s)
  assert.match(release, /<title>Kudzu 0\.12\.4 - Nested layout evidence decision<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.12\.4"/)
  assert.match(release, /Retain the app shell.*Require real pressure/s)
