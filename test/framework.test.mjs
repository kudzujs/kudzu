...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.12.1/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.12\.1 typescript/)
  assert.match(docs, /Current 0\.12\.1 application proof/)
  assert.match(examples, /Release 0\.12\.1.*v0\.12\.1.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.12\.1.*v0\.12\.1.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.12\.1".*Shared layout, history, focus, and scroll/s)
  assert.match(release, /Kudzu 0\.12\.1.*Keep the shell.*Move the route/s)
  assert.match(release, /HISTORY · FOCUS · RELEASE.*NAVIGATION LIFETIME.*npm install @kudzujs\/core@\^0\.12\.1/s)
  assert.match(release, /<title>Kudzu 0\.12\.1 - Shared layout, history, focus, and scroll<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.12\.1"/)
  assert.match(release, /Retain layout state.*Leave native routes native/s)
