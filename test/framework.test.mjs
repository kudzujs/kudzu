...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.16/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.16"/)
  assert.match(release, /Kudzu 0\.8\.16.*Source-local analysis.*Explicit compiler ownership/s)
  assert.match(release, /COMPILER ANALYSIS.*WHAT CHANGED.*npm install @kudzujs\/core@\^0\.8\.16/s)
