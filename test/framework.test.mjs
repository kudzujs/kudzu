...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.44/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.44"/)
  assert.match(release, /Kudzu 0\.8\.44.*Keep your local names.*Specialize Context privately/s)
  assert.match(release, /ALIAS · SPECIALIZE · ERASE.*CONTEXT DATAFLOW.*npm install @kudzujs\/core@\^0\.8\.44/s)
  assert.match(release, /<title>Kudzu 0\.8\.44 - Collision-free Context actions<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.44"/)
  assert.match(release, /Keep local declarations.*Keep semantic limits/s)
