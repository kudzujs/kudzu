...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.21/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.21 typescript/)
  assert.match(docs, /Current 0\.16\.21 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*11\/25 Kudzu successes.*24\/25 for React \+ Vite.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.21.*v0\.16\.21.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.21.*v0\.16\.21.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.21".*AI evidence npm recovery/s)
  assert.match(release, /Kudzu 0\.16\.21.*Keep the record.*Move only forward/s)
  assert.match(release, /NPM RECOVERY · SAME EVIDENCE · ZERO RUNTIME DELTA.*AI EVIDENCE NPM RECOVERY.*npm install @kudzujs\/core@\^0\.16\.21/s)
  assert.match(release, /<title>Kudzu 0\.16\.21 - AI evidence npm recovery<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.21"/)
  assert.match(release, /No moved tag.*Forward-only retry.*Still blocked/s)
