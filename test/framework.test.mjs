...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.16.28/index.html", import.meta.url), "utf8")
  ...
  assert.match(docs, /@kudzujs\/core@\^0\.16\.28 typescript/)
  assert.match(docs, /Current 0\.16\.28 production evidence.*benchmark:source-scale.*100,000 modules.*browser benchmarks enforce.*23\/25 Kudzu successes.*24\/25 for React \+ Vite.*50 attributable attempts.*Installed source and public web access.*138\.2 ms.*39\.6 \/ 5\.8 ms.*Kudzu 0\.16\.8.*4\.2-9\.9 KiB.*15 \/ 18/s)
  assert.match(examples, /Release 0\.16\.28.*v0\.16\.28.*KUDZU EXAMPLE CATALOG/s)
  assert.match(blog, /Kudzu 0\.16\.28.*v0\.16\.28.*source compiled by the current Kudzu release/s)
  assert.match(html, /class="release-banner" href="\/releases\/0\.16\.28".*Bounded route HTML summary/s)
  assert.match(release, /Kudzu 0\.16\.28.*Count the markers.*Verify the behavior/s)
  assert.match(release, /BOUNDED VERIFICATION.*npm install @kudzujs\/core@\^0\.16\.28/s)
  assert.match(release, /<title>Kudzu 0\.16\.28 - Bounded route HTML summary<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.16\.28"/)
  assert.match(release, /Final route HTML inventory.*Markers are not zero-JavaScript proof.*Browser tools stay repository-only.*No demonstrated AI-token advantage/s)
