...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.56/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.56"/)
  assert.match(release, /Kudzu 0\.8\.56.*Change one graph.*Leave the rest alone/s)
  assert.match(release, /TRACE · INVALIDATE · REUSE.*INCREMENTAL AFFECTED-ROUTE BUILDS.*npm install @kudzujs\/core@\^0\.8\.56/s)
  assert.match(release, /<title>Kudzu 0\.8\.56 - Incremental affected-route development builds<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.56"/)
  assert.match(release, /Trace page graphs.*Reuse route renders/s)
