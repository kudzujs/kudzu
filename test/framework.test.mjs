...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.10.3/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.10\.3".*Application-owned persistence/s)
  assert.match(release, /Kudzu 0\.10\.3.*Persist the value.*Own the policy/s)
  assert.match(release, /RESTORE · VALIDATE · RELEASE.*APPLICATION PERSISTENCE.*npm install @kudzujs\/core@\^0\.10\.3/s)
  assert.match(release, /<title>Kudzu 0\.10\.3 - Application-owned persistence<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.10\.3"/)
  assert.match(release, /Start deterministically.*Skip persistence APIs/s)
