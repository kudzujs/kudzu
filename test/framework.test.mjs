...
test("builds TSX into HTML and behavior commands without React", async () => {
  ...
  const release = await readFile(new URL("../dist/releases/0.8.32/index.html", import.meta.url), "utf8")
  ...
  assert.match(html, /class="release-banner" href="\/releases\/0\.8\.32"/)
  assert.match(release, /Kudzu 0\.8\.32.*Build away from production.*Publish only success/s)
  assert.match(release, /STAGE · VERIFY · PROMOTE.*OUTPUT OWNERSHIP.*npm install @kudzujs\/core@\^0\.8\.32/s)
  assert.match(release, /<title>Kudzu 0\.8\.32 - Staged and collision-safe output<\/title>/)
  assert.match(release, /rel="canonical" href="https:\/\/kudzujs\.cloud\/releases\/0\.8\.32"/)
  assert.match(release, /Previous output survives.*ProjectSession/s)
