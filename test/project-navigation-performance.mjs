...
async function browserRun(port) {
  ...
  try {
    ...
    await waitUntil(cdp, sessionId, 'document.readyState === "complete" && document.querySelector("[data-project=alpha]") && document.querySelector("[role=status]")?.textContent === "Projects loaded" && document.querySelector("[data-shared-project-revision]")?.textContent === "0"')
...
async function serve() {
  const source = `const http=require("node:http"),fs=require("node:fs"),path=require("node:path"),root=process.argv[1];http.createServer((request,response)=>{const pathname=new URL(request.url,"http://localhost").pathname;if(pathname==="/api/project/alpha"){response.setHeader("content-type","application/json");response.end(JSON.stringify({id:"alpha",name:"Alpha",revision:0}));return}if(pathname==="/api/projects"){response.setHeader("content-type","application/json");response.end(JSON.stringify([{id:"alpha",name:"Alpha",status:"active",issues:[{id:"a1",title:"Design schema"},{id:"a2",title:"Ship dashboard"}]},{id:"beta",name:"Beta",status:"archived",issues:[{id:"b1",title:"Archive notes"}]}]));return}const relative=pathname==="/"?"index.html":pathname.slice(1),file=path.join(root,relative.endsWith("/")||!path.extname(relative)?relative+(relative.endsWith("/")?"":"/")+"index.html":relative);response.setHeader("content-type",file.endsWith(".js")?"text/javascript":"text/html");fs.createReadStream(file).on("error",()=>response.writeHead(404).end()).pipe(response)}).listen(0,"127.0.0.1",function(){console.log(this.address().port)})`
