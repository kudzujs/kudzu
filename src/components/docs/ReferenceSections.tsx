...
export function LimitsSection() {
  return <section className="docs-section" id="limits">
    ...
    <ul className="docs-limits">
      <li>Repeated ordinary same-file and relative-imported child components own independent state IDs and effect records. Direct JSON-safe primitive parent state passed to a destructured child prop remains reactive in child DOM bindings and effect dependencies. Reactive conditional branches delete child state and clean up effects on removal, then recreate state and remount effects on re-entry. Anonymous synchronous zero-argument <code>useState</code> initializers may return directly serializable literals; derived props, captures, dynamic calls, and broader callback/ref ownership remain unsupported.</li>
