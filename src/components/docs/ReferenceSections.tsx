...
export function LimitsSection() {
  return <section className="docs-section" id="limits">
    ...
    <ul className="docs-limits">
      ...
      <li>React Router migration input supports a named or aliased <code>Link</code> used directly in JSX with exactly one static root-relative <code>to</code> and native anchor props. It also supports a named or aliased direct zero-argument <code>useParams()</code> call on a bracket route exporting <code>runtimeParams = true</code>. Kudzu applies <code>base</code> to Link destinations, redirects params to its pathname reader, and erases the package import. Dynamic, object, relative, query-only, hash-only, external destinations, <code>NavLink</code>, other router hooks, router-only props, spreads, conflicting <code>href</code>, default/namespace imports, and indirect uses are not supported. Build-known <code>getStaticPaths()</code> routes use props.</li>
