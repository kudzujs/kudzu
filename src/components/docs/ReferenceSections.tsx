...
export function LimitsSection() {
  return <section className="docs-section" id="limits">
    ...
    <ul className="docs-limits">
      ...
      <li>React Router migration input supports a named or aliased <code>Link</code> used directly in JSX with exactly one static root-relative <code>to</code> and native anchor props. It supports a direct zero-argument <code>useParams()</code> call on a <code>runtimeParams</code> bracket route and one top-level read-only <code>const [params] = useSearchParams()</code> binding whose query values each directly initialize a top-level local through <code>params.get("literal")</code>. Kudzu applies <code>base</code> to Link destinations, redirects path/query reads to route-specific capabilities, and erases the package import. Dynamic Link destinations, <code>NavLink</code>, query setters, dynamic names, other URLSearchParams methods, aliases, wrapped/indirect reads, other router hooks, router-only props, spreads, conflicting <code>href</code>, and default/namespace imports are not supported. Build-known <code>getStaticPaths()</code> routes use props.</li>
