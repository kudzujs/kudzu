...
export function LimitsSection() {
  return <section className="docs-section" id="limits">
    ...
    <ul className="docs-limits">
      <li>Repeated ordinary same-file and relative-imported child components own independent state IDs and effect records. Direct JSON-safe primitive parent state passed to a destructured child prop remains reactive in child DOM bindings and effect dependencies. An inline or simple <code>const</code> setter callback and a <code>null</code>-initialized object ref may cross one boundary to a direct intrinsic root. Reactive conditional branches drop handlers and refs with removed DOM, delete child state, and clean up effects, then recreate ownership on re-entry. Derived props, arbitrary callbacks, callback refs, captures, dynamic calls, and further forwarding remain unsupported.</li>
      ...
      <li>Keyed lists require a local array-state anchor and intrinsic roots or specialized same-file/relative row components. Directly exported same-file rows may be reused at static and keyed JSX sites. Immutable local collection aliases may compose inline pure <code>filter</code>, direct-property <code>flatMap</code>, and <code>Array.from</code> selectors and feed multiple keyed list sites when every reference is an analyzable collection source. Callbacks accept <code>(item)</code> or <code>(item, index)</code>; field keys own item identity and index keys own positional identity.</li>
      <li>A keyed row may contain multiple sibling maps over direct parent-item array properties recursively without a numeric depth limit. Intrinsic, same-file, directly exported same-file, or relative-imported rows support recursive specialization, analyzable prop spreads, directly serializable literal prop defaults, direct intrinsic rest forwarding, forwarded JSX children, nested conditions, latest-item handlers, multiple directly serializable state slots, supported effects, and <code>null</code>-initialized object refs.</li>
      <li>Computed child collections, parent capture in child rows, component cycles, package or namespace row imports, export-list/default aliases, non-JSX component references, dynamic/computed prop spreads or defaults, indirect/repeated rest use, rest-forwarded children, derived-expression captures, dynamic row state, non-<code>null</code> refs, and arbitrary or asynchronous collection callbacks remain unsupported.</li>
      ...
      <li><code>useEffect</code> supports inline block-bodied callbacks, directly returned inline cleanup, DOM ownership in ordinary conditional children and supported keyed rows, and literal arrays of direct JSON-safe primitive state or runtime parameter identifiers. Destructured ordinary child props passed directly from parent state retain that dependency; keyed rows also accept direct <code>item.&lt;field&gt;</code> properties whose values remain JSON-safe primitives. Whole-item, computed, nested, derived, prototype-sensitive, non-item property, ordinary local, object, spread, dynamic, named or dynamic cleanup, cleanup parameter or generator, async-cleanup, and other return forms are not supported.</li>
