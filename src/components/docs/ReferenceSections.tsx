...
export function ArchitectureSection() {
  ...
  -> complete HTML + route-specific capability ESM`} />
    ...
    <p>The compiler uses TypeScript AST transforms through <code>transpileModule()</code>; semantic type checking remains the separate <code>tsc --noEmit</code> project check. Each build owns an explicit ProjectSession containing its root, source records, graph operations, compiler paths, and Worker compiler. Browser runtime is capability-specific rather than absent: a route receives only the command, binding, list, effect, native-handler, parameter, Worker, or navigation modules it needs. The <a href="https://github.com/kudzujs/kudzu/tree/v0.8.33/docs/next-architecture">architecture packet</a> records the current compiler and ordered large-application program.</p>
  </section>
}
...
CHROME_BIN="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \\
  node --test --test-name-pattern="bundles relative TypeScript Workers" test/framework.test.mjs`} />
    ...
    <div className="docs-callout"><strong>Historical comparison</strong><span>This dated 0.7.12 snapshot retains raw arrays in PERFORMANCE.md. It is not a current 0.8.33 framework ranking.</span></div>
