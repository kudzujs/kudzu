...
export function LimitsSection() {
  return <section className="docs-section" id="limits">
    ...
    <ul className="docs-limits">
      ...
      <li>Keyed lists require local-state maps and intrinsic roots or top-level local or relative-imported row components. One nested keyed map may read a direct array property of its parent item and use an intrinsic, same-file, or relative-imported child row with one item-local condition level. Same-file or relative default, named/aliased, and direct named re-export wrappers may receive the state identifier directly and render the outer map under an intrinsic root. Package, namespace, and star-export wrappers, package and namespace row imports, reusable aliases, computed nested collections, parent capture in child rows, multiple or deeper nested lists, conditions inside child conditions, child effects or local state, component tags below the specialized child root, prop spreads, and derived-expression captures remain unsupported.</li>
