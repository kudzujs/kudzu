...
export function LimitsSection() {
  return <section className="docs-section" id="limits">
    ...
    <ul className="docs-limits">
      <li>Repeated ordinary same-file and relative-imported child components own independent state IDs and effect records. Direct JSON-safe primitive parent state passed to a destructured child prop remains reactive in child DOM bindings and effect dependencies. A direct setter may cross one boundary when invoked once by a direct intrinsic event handler; inline/simple <code>const</code> setter callbacks may use that adapter shape or direct forwarding, and the child may own directly serializable <code>useState()</code>, direct primitive prop <code>.toString()</code> string-state initialization, <code>useId()</code>, supported effects, and <code>null</code>-initialized object refs. A parent-owned object ref may cross the same boundary. Reactive conditional branches drop handlers and refs with removed DOM, delete child state, and clean up effects, then recreate ownership on re-entry. Other dynamic state initializers, derived props, arbitrary callbacks, callback refs, multiple callback uses, captures, dynamic calls, and further forwarding remain unsupported.</li>
      <li>Reactive JSX text and attributes may reference recursively chained top-level immutable locals derived through supported pure primitive expressions from direct state. Kudzu substitutes the expressions and subscribes each source state. Arbitrary calls, mutation, cycles, shadowed or block-local declarations, and structural JSX locals remain unsupported.</li>
      ...
      <li>Reactive conditionals and flat intrinsic keyed lists are supported inside SVG and preserve the SVG namespace for inserted nodes. Keyed-item conditions, nested SVG keyed lists, reactive MathML structures, and namespaced SVG attributes such as <code>xlinkHref</code> are not yet supported.</li>
      ...
      <li>React Router migration input supports a named or aliased <code>Link</code> used directly in JSX with exactly one static root-relative <code>to</code> and native anchor props. It supports a direct zero-argument <code>useParams()</code> call on a <code>runtimeParams</code> bracket route, one top-level read-only <code>const [params] = useSearchParams()</code> binding with direct static <code>get()</code> locals, and one top-level <code>useNavigate()</code> binding called directly from nested browser callbacks with safe static root-relative destinations. Kudzu applies <code>base</code>, redirects path/query reads to route-specific capabilities, lowers navigation to <code>location.assign()</code> or exactly <code>{`{ replace: true }`}</code> to <code>location.replace()</code>, and erases the package import. Dynamic destinations, <code>NavLink</code>, query setters, other URLSearchParams methods, navigate aliases, render-time navigation, other navigation options, other router hooks, router-only props, spreads, conflicting <code>href</code>, and default/namespace imports are not supported. Build-known <code>getStaticPaths()</code> routes use props.</li>
