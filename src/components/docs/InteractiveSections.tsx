...
return <main>{categories.map(category =>
  <section key={category.id}>
    ...
    <p>Specialized rows accept direct same-file exports reused across static and keyed JSX sites, JSX children, prop spreads from inline objects or one direct <code>const</code> object resolved through calling-component scopes, directly serializable literal defaults, and one final rest binding forwarded exactly once to the direct intrinsic root. Package or namespace row imports, export-list/default aliases, non-JSX component references, dynamic/computed prop spreads or defaults, indirect/repeated rest use, rest-forwarded children, computed child collections, parent capture, component cycles, fragments, lazy/dynamic state initializers, callback refs, mutation, arbitrary collection callbacks, and <code>dangerouslySetInnerHTML</code> remain unsupported. Put keyed rows inside an explicit <code>tbody</code>, <code>thead</code>, or <code>tfoot</code>.</p>
  </section>
}
