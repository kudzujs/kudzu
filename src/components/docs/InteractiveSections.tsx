...
return <main>{categories.map(category =>
  <section key={category.id}>
    ...
    <p>Each item and nested object must be an ordinary JSON-safe plain object with a unique string or finite-number key. Direct item reads use compact markers; pure synchronous derived text, attributes, object styles, and nested item conditions compile to external evaluators. A flat row text or attribute expression may combine its item or index with direct primitive parent state, supporting selected-row classes and ARIA attributes while retaining keyed DOM identity. An expression-bodied callback with one item parameter may also return <code>condition &amp;&amp; &lt;Row /&gt;</code> or <code>condition ? &lt;Row /&gt; : null</code>; Kudzu lowers the pure item/state condition to its existing filter path so removal cleans up row ownership and re-entry starts fresh. Immutable local aliases may compose inline pure <code>filter</code>, direct-property <code>flatMap</code>, <code>Array.from</code>, immutable <code>slice(start, end?)</code>, and <code>toSorted((left, right) =&gt; expression)</code> and feed multiple keyed list sites when every alias reference is an analyzable collection source. Filter predicates may combine pure string methods and direct primitive state for reactive search; slice bounds may use numeric literals or supported state expressions for pagination. Sorting evaluates a constrained two-parameter comparator over a copy and rejects mutating <code>sort()</code>. A relative-imported named or default synchronous one-parameter function may directly return the same pure pipeline; Kudzu composes its selector without executing or shipping the function. Ordinary map callbacks may use <code>(item, index)</code>, and <code>key={`{index}`}</code> intentionally owns DOM by position; conditional-root maps reject indexes because filtering changes their meaning.</p>
...
export function EventsSection() {
  return <section className="docs-section" id="events">
    ...
    <p>When keyed or conditional removal, enhanced navigation, or document disposal unmounts a native handler's DOM owner, pending async work loses state-write, queued-commit, and captured-ref authority. Late setters and commits become no-ops, and refs resolve to <code>null</code> instead of a replacement node that reused the same ownership ID. Promises and ordinary reads may finish; Kudzu adds no scheduler.</p>
    <p>Native handlers may call default, named, or namespace helpers from relative TypeScript modules. Kudzu bundles the reachable helper graph into handler ESM and shared chunks. Helper runtime imports must remain relative; package imports inside helper graphs, dynamic imports, JSX helpers, and direct imported event callbacks are rejected. A package import is supported when referenced directly inside an intrinsic JSX event callback or inline owned effect setup and cleanup callback.</p>
    ...
    <p>Object refs resolve DOM elements while their native handler owner is mounted. They initialize with <code>null</code>, resolve to <code>null</code> after ownership release, and need no effect or hydration lifecycle.</p>
