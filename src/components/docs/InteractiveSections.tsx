...
return <main>{categories.map(category =>
  <section key={category.id}>
    ...
    <p>Package or namespace row imports, same-file exported rows, reusable aliases, prop spreads/rest, non-primitive defaults, children props, computed child collections, parent capture, component cycles, fragments, lazy/dynamic state initializers, callback refs, mutation, arbitrary collection callbacks, and <code>dangerouslySetInnerHTML</code> remain unsupported. Put keyed rows inside an explicit <code>tbody</code>, <code>thead</code>, or <code>tfoot</code>.</p>
  </section>
}
