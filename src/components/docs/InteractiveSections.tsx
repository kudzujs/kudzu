...
return <><button onClick={create}>Create</button><button onClick={add}>Add</button><button onClick={reverse}>Reverse</button><ul>{rows}</ul></>`} />
    <p>A top-level same-file row component or one imported from a relative TypeScript module can be the map root. Default, named, aliased, and named re-export imports resolve at build time. Kudzu specializes each call, supporting projected props, callback props, and simple local calculations without shipping a browser component runtime.</p>
