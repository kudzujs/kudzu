...
export default function Page() {
  return <Greeting name="Kudzu" />
    ...
    <p>Ordinary same-file and relative-imported child components may own local state without specialization into a browser component. Repeated calls receive independent state IDs even when they share one generated handler module. When a stateful child is inside reactive conditional DOM, its branch owns the state slots: removal deletes them and re-entry creates fresh values from the original initializer. The initial visible branch reuses its pre-rendered IDs instead of executing the component twice.</p>
