...
- Keyed local-state and imported static collections, pure selectors, nested direct-property lists, recursive row specialization, latest-item handlers, row state/effects/refs, and stable keyed identity.
...
- ordinary prop spreads with statically known sources;
- non-primitive destructured defaults and rest where values remain analyzable;
- wrapper and layout components forwarding `children`;
