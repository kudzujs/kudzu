...
- Imported relative TypeScript helpers, imported keyed-list row components, and same-file or relative-imported state-backed list wrappers already have build-time resolution paths. One direct parent-property child map may also specialize a same-file or relative-imported row with latest-item handlers and one child-local condition level.

The TodoMVC React implementation at `tastejs/todomvc@ff43b02` supplied the first post-Goal-B migration blockers: reducer-owned todo state, dispatch passed to child controls and keyed todo rows, keyed-row edit state, a control callback passed to an input, and the input's `editing = false` prop default. Kudzu accepts direct `[state, dispatch] = useReducer(reducer, initialValue)` with a pure synchronous two-parameter default or named reducer imported from a relative TypeScript module. Dispatch lowers to the existing functional state update path, so sequential actions observe current logical state and DOM work still batches once per turn. One direct prop boundary into a same-file or relative-imported synchronous component is specialized at the parent call site; its intrinsic handler uses the parent reducer metadata, no dispatch is serialized, and the imported child's dead handler asset is not emitted. A direct keyed row may receive its item and dispatch; inline and simple `const` event handlers read the latest item through the existing list scope. The row may declare one top-level primitive-literal `useState`; its hook slot and list key produce a concrete state ID used by existing event, binding, and conditional capabilities. Item updates and reorder preserve the state, while removal deletes it and a later re-add starts from the initializer. Relative TypeScript constants and helpers used inside those handlers receive collision-free aliases and join the parent handler graph. One inline or simple `const` callback containing dispatch may cross one additional relative-imported synchronous component boundary into an intrinsic event handler; that child is also specialized, may use relative TypeScript handler helpers and primitive literal prop defaults, and has its handler asset omitted. The optimized edit, reorder, removal, and re-add fixture emits 21,803 B raw/8,968 B gzip across seven JavaScript files; seven clean builds measured 383.8-415.2 ms with a 396.3 ms median. No reducer-specific runtime is emitted. Object, array, computed, and call defaults, multiple or non-keyed specialized local states, lazy state or reducer initialization, reducer-dispatch keyed-row effects, package imports, child imports outside handlers, `useCallback`, further forwarding, and reducer dispatch through context remain outside this reduced slice.

The matched 1,000-row local-state benchmark edits row 500, reverses while requiring the same row and input DOM nodes, removes it, and re-adds the same key with reset state. Seven fresh-profile medians measured Kudzu at 2.7/10.1/3.0/3.1 ms for edit/reverse/remove/re-add, React at 5.9/26.4/9.0/6.5 ms, Vue at 2.8/11.7/4.0/4.2 ms, Svelte at 2.6/47.9/4.5/5.4 ms, and hand-written Astro/native at 1.6/5.9/1.2/1.4 ms. Kudzu's 449 ms clean-build median was the lowest framework build. Kudzu and Astro emit initial rows while the other fixtures are CSR; Kudzu's 570.7 KB output includes complete HTML and per-row descriptors, so artifact sizes are not architecture-equivalent.

The matched 100-parent/1,000-child component benchmark updates one child's text and condition, reverses its children and parents independently, and removes one parent while requiring existing DOM identity. Kudzu measured 1.4/0.5/5.4/0.7 ms for those operations and a 403 ms clean-build median.

Initial child rows retain complete current HTML while their lists share one child row prototype, inert condition branches, and patch descriptors. This reduces HTML from 676,086 B to 339,586 B and total raw deploy output from 693,806 B to 359,035 B. Sharing text and attribute patch metadata saves 29,576 B raw over `v0.6.26` while adding 147 B to aggregate gzip. Ordinary flat-list HTML remains byte-for-byte unchanged, and nested marker lookup is compiled out when nested lists are absent.
