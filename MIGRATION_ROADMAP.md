...
The completed 0.9 plan defines the cross-framework comparison and release gate. The active application packet is `0.21.3` in the capability release plan below. Benchmark-only feature omission, unmatched accessibility, weighted scores that hide losses, and unrecorded environment differences do not count as proof.
