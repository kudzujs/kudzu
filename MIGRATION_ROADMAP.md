...
The TodoMVC React implementation at `tastejs/todomvc@ff43b02` supplied the first post-Goal-B migration blockers: reducer-owned todo state, dispatch passed to child controls and keyed todo rows, a control callback passed to an input, and the input's `editing = false` prop default. Kudzu accepts direct `[state, dispatch] = useReducer(reducer, initialValue)` with a synchronous two-parameter default or named reducer imported from a relative TypeScript module. Dispatch lowers to the existing functional state update path, so sequential actions observe current logical state and DOM work still batches once per turn. One direct prop boundary into a same-file or relative-imported synchronous component is specialized at the parent call site; its intrinsic handler uses the parent reducer metadata, no dispatch is serialized, and the imported child's dead handler asset is not emitted. A direct keyed row may receive its item and dispatch; inline and simple `const` event handlers read the latest item through the existing list scope. Relative TypeScript constants and helpers used inside those handlers receive collision-free aliases and join the parent handler graph. One inline or simple `const` callback containing dispatch may cross one additional relative-imported synchronous component boundary into an intrinsic event handler; that child is also specialized, may use relative TypeScript handler helpers and primitive literal prop defaults, and has its handler asset omitted. The keyed-row removal fixture emits 17,356 B raw/7,602 B gzip across seven JavaScript files; seven clean builds measured 366.5-402.2 ms with a 382.7 ms median. No reducer-specific runtime is emitted. Object, array, computed, and call defaults, reducer-dispatch keyed-row local state or effects, lazy reducer initialization, package imports, child imports outside handlers, `useCallback`, further forwarding, and reducer dispatch through context remain outside this reduced slice.
