...
The first landing-page migration fixture retains `import React, { useState } from "react"`, `React.Fragment`, and named `Fragment` across relative components. The compiler rewrites those imports to Kudzu before module evaluation, emits no React reference, leaves its static route JavaScript-free, and compiles only the interactive route's mobile-menu state, attribute, text, condition, and handler capabilities. Named supported hooks must currently retain their imported names; default and namespace imports are accepted for `Fragment`, not member hook calls. The two-route fixture emits 2,408 B HTML and 10,245 B raw/5,030 B aggregate gzip JavaScript across seven capability files; after one warm-up, seven clean builds measured 304.0-326.3 ms with a 310.0 ms median. This is source ingestion, not a React runtime or compatibility layer.
