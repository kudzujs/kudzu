...
The first landing-page migration fixture retains `import React, { useState } from "react"`, `React.Fragment`, and named `Fragment` across relative components. It also retains Vite-shaped side-effect CSS, scoped CSS Module classes, default image/SVG/font imports, asset `?url`, and relative CSS `url(...)` references under a non-root base. The compiler rewrites React imports to Kudzu, resolves styles and assets to build-time literals, and copies referenced bytes under source-relative `dist/assets` paths. The same lowering applies inside specialized imported keyed rows. It emits no React reference, leaves the static route JavaScript-free, and compiles only the interactive route's mobile-menu state, attribute, text, condition, and handler capabilities. Named supported hooks must currently retain their imported names; default and namespace imports are accepted for `Fragment`, not member hook calls. CSS Module `composes`, arbitrary import queries, and named or namespace asset bindings remain unsupported. This is source ingestion, not a React runtime or compatibility layer.

The TodoMVC React implementation at `tastejs/todomvc@ff43b02` supplied the first post-Goal-B reducer blockers: reducer-owned todo state, dispatch passed to child controls and keyed rows, a callback passed to an input, and a primitive prop default. Kudzu accepts direct `[state, dispatch] = useReducer(reducer, initialValue)` with a pure synchronous two-parameter reducer imported from a relative TypeScript module. Dispatch may cross one direct specialized same-file/relative component boundary, including a keyed row, and one inline or simple `const` callback may cross one additional relative-imported intrinsic boundary. Keyed reducer rows now use the general row specialization for multiple serializable state values, nested conditions, effects, and `null`-initialized object refs. Relative TypeScript handler helpers join the parent graph; no reducer runtime, dispatch capture, or dead child handler asset is emitted. Lazy state/reducer initialization, non-keyed specialized local state, package reducers, child imports outside handlers, `useCallback`, further forwarding, and reducer dispatch through context remain unsupported. The original optimized fixture emitted 21,803 B raw/8,968 B gzip across seven JavaScript files and had a 396.3 ms seven-build median.
