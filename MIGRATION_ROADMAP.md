...
The matched 100-parent/1,000-child component benchmark updates one child's text and condition, reverses its children and parents independently, and removes one parent while requiring existing DOM identity. Kudzu measured 1.2/0.4/4.6/0.6 ms for those operations and a 430 ms clean-build median. Initial child rows retain complete current HTML but share inert condition branches and descriptors through each live child-list prototype, reducing HTML from 676,086 B to 417,086 B and total raw deploy output from 693,806 B to 435,326 B. Ordinary flat-list output remains byte-for-byte unchanged.
