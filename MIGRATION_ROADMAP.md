...
The TodoMVC React implementation at `tastejs/todomvc@ff43b02` supplied the first post-Goal-B migration blockers: reducer-owned todo state and dispatch passed to child controls. Kudzu accepts direct `[state, dispatch] = useReducer(reducer, initialValue)` with a synchronous two-parameter default or named reducer imported from a relative TypeScript module. Dispatch lowers to the existing functional state update path, so sequential actions observe current logical state and DOM work still batches once per turn. One direct prop boundary into a same-file or relative-imported synchronous component is specialized at the parent call site; its intrinsic handler uses the parent reducer metadata, no dispatch is serialized, and the imported child's dead handler asset is not emitted. Relative TypeScript constants and helpers used inside that handler receive collision-free aliases and join the parent handler graph. The expanded empty keyed-list fixture emits 15,596 B raw/7,011 B gzip across its complete JavaScript graph; seven clean builds measured 326.6-425.6 ms with a 366.4 ms median. No reducer-specific runtime is emitted. Lazy initialization, package imports, child imports outside handlers, second-hop forwarding, and reducer dispatch through context remain outside this reduced slice.
