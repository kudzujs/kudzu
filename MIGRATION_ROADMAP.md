...
The matched 100-parent/1,000-child component benchmark updates one child's text and condition, reverses its children and parents independently, and removes one parent while requiring existing DOM identity. Kudzu measured 1.1/0.4/4.7/0.7 ms for those operations and a 392 ms clean-build median.

Initial child rows retain complete current HTML while their lists share one child row prototype, inert condition branches, and descriptors. This reduces HTML from 676,086 B to 369,586 B and total raw deploy output from 693,806 B to 388,611 B. The final prototype step saves 46,715 B raw over `v0.6.25` while adding 447 B to aggregate gzip. Ordinary flat-list HTML remains byte-for-byte unchanged, and prototype lookup is compiled out when nested lists are absent.
