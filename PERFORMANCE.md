...
The completed frozen C2 rerun expands that stateful matrix to immutable object replacement, multi-state derived output, keyed row-local state/effect/ref ownership, filter/restore, conditional cleanup/fresh remount, accessible SVG keyboard state, a static sibling, and blocked/delayed/missing-JavaScript acceptance. Across seven fresh profiles every target passes all 19 operations and ownership checks. Kudzu emits 41,862 raw / 15,819 gzip B JavaScript versus 196,089 / 60,661 B React, 66,620 / 25,624 B Vue, 44,046 / 16,818 B Svelte, and 197,771 / 61,994 B Astro+React. The completed C5 rerun records 32 balanced Worker starts, invalidations, listener removals, terminations, and ignored late callbacks after replacement plus 30 mount/dispose cycles for every target, with zero stale mutations and zero final active handles. Kudzu emits 17,639 raw / 8,189 gzip B for that Worker application versus 193,422 / 60,159 B React and 195,108 / 61,493 B Astro+React. Uniform CPU/heap/transfer/build-RSS fields remain incomplete, so these are acceptance and byte records rather than a final release claim.

The public commerce fixture compared Kudzu and Astro with acceptance before timing and seven rotating cold/warm samples. At 100 products Kudzu cold median is 3,317 ms versus Astro 3,558 ms. At 1,000 products import-free graph discovery, used-import semantic resolution, repeated capability projection removal, and streamed scratch JSON reduce Kudzu's cold median from 7,874 to 6,248 ms and median peak RSS from 743,512 to 646,472 KiB while preserving 1,011 pages, 10,164,835 output bytes, and digest `61eb1b7c633a2facabb782bc654a5ad07169aeb82cf47dbccda9b9e7303a716e`. Astro records 5,985 ms cold, 5,355 ms warm, and 409,868 KiB: Kudzu's 4.4% cold loss is below the materiality threshold, but its 13.2% warm and 57.7% RSS losses remain release blockers. The overlapping timing ranges require 21 samples before a final timing conclusion, and 10,000-route measurement remains deferred until the 1,000-route gate passes.
